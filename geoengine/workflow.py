...
class Workflow:
    ...
    def __get_wfs_url(self, bbox: QueryRectangle) -> str:
        ...
        wfs_url = (
            req.Request(
                "GET",
                url=f"{session.server_url}/wfs/{self.__workflow_id}",
                params=params,
            )
            .prepare()
            .url
    ...
    def get_wfs_get_feature_curl(self, bbox: QueryRectangle) -> str:
        ...
        wfs_request = req.Request(
            "GET", url=self.__get_wfs_url(bbox), headers=get_session().auth_header
        ).prepare()
        ...
        return command.format(
            method=wfs_request.method, headers=headers, uri=wfs_request.url
        )
    ...
    def wms_get_map_as_image(
        self, bbox: QueryRectangle, colorizer_min_max: str 
    ) -> Image:
    ...
    def __wms_get_map_request(
        ...
        params = dict(
            ...
            height=height,
            format="image/png",
            styles=colorizer_min_max,  # TODO: incorporate styling properly
        )
    ...
    def wms_get_map_curl(
        self, bbox: QueryRectangle, colorizer_min_max: Tuple[float, float] = None
    ) -> str:
        ...
        headers = " -H ".join(headers)
        return command.format(
            method=wms_request.method, headers=headers, uri=wms_request.url
        )
    ...
    def plot_chart(self, bbox: QueryRectangle) -> VegaLite:
        ...
        plot_url = (
            f"{session.server_url}/plot/{self}?bbox={spatial_bounds}&crs={bbox.srs}&time={time}"
    ...
    def __get_wcs_tiff_as_memory_file(
        self, bbox: QueryRectangle, timeout=3600
    ...
    def get_provenance(self) -> List[ProvenanceOutput]:
        ...
        provenance_url = (
            f"{session.server_url}/workflow/{self.__workflow_id}/provenance"
        )
    ...
    def save_as_dataset(
        self, bbox: QueryRectangle, name: str, description: str = ""
    ) -> StoredDataset:
        ...
        if self.get_result_descriptor().spatial_reference != bbox.srs:
            raise SpatialReferenceMismatchException(
                self.get_result_descriptor().spatial_reference, bbox.srs
            )
...
def register_workflow(workflow: Dict[str, Any]) -> Workflow:
    ...
    workflow_response = req.post(
        f"{session.server_url}/workflow", json=workflow, headers=session.auth_header
    ).json()
